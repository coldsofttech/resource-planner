...
_PROJECT_TYPE_PROTECTED_ERROR = (
    "This project type is protected and cannot be modified or deleted."
)
...
class ProjectTypeService(AuditableService, FilterableQueryService):
    _MODULE = "projects"
    _RESOURCE_TYPE = "project_type"
    ...
    def get_queryset(self):
        return selectors.get_all_project_types()

    def _snapshot(self, project_type: ProjectType) -> dict:
        return {
            "code": project_type.code,
            "name": project_type.name,
            "description": project_type.description,
            "is_active": project_type.is_active,
            "is_protected": project_type.is_protected,
        }

    def get(self, code: str, *args, **kwargs) -> ProjectType:
    ...
    def create(
        ...
        *,
        ...
        if selectors.project_type_exists(name):
            raise AlreadyExistsException(
                detail=f"A project type named '{name}' already exists."
            )
        obj = ProjectType.objects.create(
            name=name,
            description=description,
            is_active=is_active,
            is_protected=False,
    ...
    @transaction.atomic
    def update(self, code: str, **kwargs) -> ProjectType:
        obj = self.get(code=code)
        if obj.is_protected:
            raise ValidationException(_PROJECT_TYPE_PROTECTED_ERROR)

        before = self._snapshot(obj)
        update_fields: list[str] = ["updated_by", "updated_at"]

        new_name = kwargs.get("name", obj.name)
        if new_name != obj.name and selectors.project_type_exists(
            new_name, exclude_pk=obj.pk
        ):
            raise AlreadyExistsException(
                detail=f"A project type named '{new_name}' already exists."
            )
    ...
    @transaction.atomic
    def activate(self, code: str) -> ProjectType:
        obj = self.get(code=code)
        if obj.is_protected:
            raise ValidationException(_PROJECT_TYPE_PROTECTED_ERROR)
        if not obj.is_active:
    ...
    @transaction.atomic
    def deactivate(self, code: str) -> ProjectType:
        obj = self.get(code=code)
        if obj.is_protected:
            raise ValidationException(_PROJECT_TYPE_PROTECTED_ERROR)
        if obj.is_active:
    ...
    def delete(self, code: str, *args, **kwargs) -> None:
        ...
        if obj.is_protected:
            raise ValidationException(_PROJECT_TYPE_PROTECTED_ERROR)
        project_type_code = obj.code
        before = self._snapshot(obj)
        obj.delete()
        AuditService.log_delete(
            module=self._MODULE,
            resource_type=self._RESOURCE_TYPE,
            resource_code=project_type_code,
            before=before,
            actor=self.user,
        )
    ...
    def options(self) -> list[dict]:
        return [
            ...
            for pt in selectors.get_project_type_options()
        ]

    def stats(self, fields=None, *args, **kwargs) -> dict:
        all_stats = selectors.get_project_type_stats()
        if fields:
            return {k: v for k, v in all_stats.items() if k in fields}
        return all_stats


class ProjectTypeImportService(ImportService):
    SUPPORTED_IMPORT_FORMATS = ["csv"]
    MAX_IMPORT_ROWS = 1_000
    MAX_IMPORT_FILE_SIZE_MB = 5
            ...
            errors.append(
                {
                    "row": row_num,
                    "field": "name",
                    "message": "Project type name must be 60 characters or fewer.",
                }
            )
        return errors
    ...
    def bulk_import(self, file, dry_run: bool = False) -> dict:
        ...
        created_rows: list[dict] = []
        errors: list[dict] = []
        svc = ProjectTypeService(user=self.user)
                ...
                errors.append(
                    {
                        "row": row_num,
                        "field": "name",
                        "message": f"A project type named '{name}' already exists.",
                    }
                )
                continue
...
class ProjectTypeExportService(ExportService):
    EXPORT_FIELD_LABELS: dict[str, str] = {
        "code": "Code",
    ...
    EXPORT_FILENAME = "project_types_export"
    EXPORT_MODULE_NAME = "Project Types"
    ...
    def export(
        ...
        qs = selectors.get_all_project_types()
        if filters:
            is_active_raw = filters.get("is_active")
            if is_active_raw in (None, ""):
                qs = qs.filter(is_active=True)
