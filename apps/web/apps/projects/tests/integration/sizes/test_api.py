...
class ProjectSizeConfigPatchAPITest(TestCase):
    ...
    def test_updated_values_reflected_in_response(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(PATCH_URL, _VALID_PAYLOAD, format="json")
        data = response.data["data"]
    ...
    def test_updated_values_persist_on_subsequent_get(self):
        self.client.force_authenticate(user=self.user)
    ...
    def test_missing_xs_max_amount_returns_400(self):
        self.client.force_authenticate(user=self.user)
