...
// /sprints/SPRINT-x/actuals/SPTIMP-y/  →  parts[1]=SPRINT-x, parts[3]=SPTIMP-y
...
window.renderActualsImportRowDetail = function renderActualsImportRowDetail(row) {
...
window.renderActualsImportCapacityRow = function renderActualsImportCapacityRow(row) {
...
function enablePostReviewUI() {
  ...
  const confirmBtn = document.getElementById("rp-actuals-import-confirm-btn");
  if (confirmBtn) confirmBtn.removeAttribute("disabled");

  const table = document.getElementById("rp-actuals-import-capacity-table");
  if (table && sprintCode && teamCode && !table.getAttribute("url")) {
    ...
    const listView = document.getElementById("rp-actuals-import-capacity-list");
...
async function loadImportDetail() {
  ...
  try {
    const { href, method } = API_URLS.sprints.actualsImportDetail(sprintCode, importCode);
    ...
    const titleEl = document.getElementById("rp-actuals-import-title");
    if (titleEl) titleEl.textContent = `${teamName} | ${sprintName} — Actuals`;
    ...
    setBreadcrumbs([
      ...
      { label: "Actuals", href: UI_URLS.sprints.actuals(sprintCode) },
      { label: `v${record.version_number}` },
    ]);

    const reviewBtn = document.getElementById("rp-actuals-import-review-btn");
...
function initRowsTable(table) {
  if (!sprintCode || !importCode) return;
  const baseUrl = API_URLS.sprints.actualsImportRows(sprintCode, importCode).href;
  table.setAttribute("url", baseUrl);

  const listView = document.getElementById("rp-actuals-import-rows-list");
  if (listView) listView._baseUrl = baseUrl;
}

function initReviewButton(table) {
  const btn = document.getElementById("rp-actuals-import-review-btn");
  if (!btn) return;
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      const { href, method } = API_URLS.sprints.actualsImportReview(sprintCode, importCode);
      const resp = await apiFetch(href, { method });
      ...
      const capacityTable = document.getElementById("rp-actuals-import-capacity-table");
...
function openDeleteModal(row) {
  const modal = document.getElementById("rp-actuals-import-row-delete-modal");
  if (!modal) return;
  pendingRow = row;
...
function initDeleteModal(table) {
  ...
  modal.addEventListener("rp:delete", async () => {
    ...
    const { href, method } = API_URLS.sprints.actualsImportRowDelete(
      sprintCode,
      importCode,
      pendingRow.code,
    );
...
function initAddRowButton(table) {
  const addBtn = document.getElementById("rp-actuals-import-add-row-btn");
  const drawer = document.getElementById("rp-actuals-import-add-row-drawer");
  ...
  drawer.addEventListener("rp:footer-primary", async () => {
    ...
    try {
      const { href, method } = API_URLS.sprints.actualsImportRowCreate(sprintCode, importCode);
...
function openEditDrawer(row) {
  const drawer = document.getElementById("rp-actuals-import-edit-row-drawer");
  if (!drawer) return;
...
function initEditDrawer(table) {
  ...
  drawer.addEventListener("rp:footer-primary", async () => {
    ...
    try {
      const { href, method } = API_URLS.sprints.actualsImportRowUpdate(
        sprintCode,
        importCode,
        pendingRow.code,
      );
...
function lockImportUI() {
  [
    "rp-actuals-import-confirm-btn",
    "rp-actuals-import-review-btn",
    "rp-actuals-import-add-row-btn",
  ].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.setAttribute("disabled", "");
  });

  const table = document.getElementById("rp-actuals-import-rows-table");
  if (!table) return;
...
function initConfirmButton() {
  const btn = document.getElementById("rp-actuals-import-confirm-btn");
  const drawer = document.getElementById("rp-actuals-import-confirm-drawer");
  if (!btn || !drawer) return;

  btn.addEventListener("click", () => {
    const notesField = document.getElementById("rp-actuals-confirm-import-notes");
    const msgEl = document.getElementById("rp-actuals-confirm-import-message");
  ...
  drawer.addEventListener("rp:footer-primary", async () => {
    ...
    try {
      const { href, method } = API_URLS.sprints.actualsImportConfirm(sprintCode, importCode);
      await apiFetch(href, {
        method,
      ...
      toast({
        ...
        message: "This actuals import has been confirmed.",
      });
    } catch (err) {
      restoreButton(submitBtn, snap);
