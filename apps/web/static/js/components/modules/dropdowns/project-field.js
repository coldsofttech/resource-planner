 ...
 *
 * Dropdown (or chip-based multi-select) field pre-wired to the projects API.
 * Active projects are fetched from GET /api/v1/projects/options/ on first
 ...
 *                  Ignored when multi-select is set.
 *   allow-all    – single-select only; prepends an "All Projects" option
 *                  (value="") selected by default; used in filter contexts.
 *                  Ignored when multi-select is set.
 *   show-label   – when present, renders "Project" as the visible field label.
 *   multi-select – when present, renders as a chip-based multi-select instead
 *                  of a dropdown. value getter returns a JSON array string;
 *                  setter accepts a JSON array string, comma-separated string,
 *                  or Array.
 *
 * Usage (single-select):
 ...
 *
 * Usage (multi-select):
 *   <project-field id="group-projects" multi-select col="col-12"></project-field>
 *   <project-field id="group-projects" multi-select value='["PROJ-1","PROJ-2"]'></project-field>
 *
 * Reading the multi-select value in JS:
 *   JSON.parse(field.value)  // → ["PROJ-1", "PROJ-2"]
 *   field.values             // → [{id, label, value}, …]
 */
class ProjectField extends DropdownField {
  static get observedAttributes() {
    return [...super.observedAttributes, "show-label", "programme-id", "multi-select"];
  }
  ...
  attributeChangedCallback(name, oldVal, newVal) {
    if (this._isMultiSelect && name === "value" && this._connected && oldVal !== newVal) {
      this.value = newVal || "";
    } else if (name === "programme-id" && oldVal !== newVal && this.isConnected) {
  ...
  // In multi-select mode _selectedValues is maintained in memory — no DOM value to save/restore.
  _savedValue() {
  ...
  connectedCallback() {
    ...
    if (firstConnect) {
      ...
      if (this._isMultiSelect) {
        // Seed _selectedValues with placeholder chips (code as label) so
        // pre-selected values are visible immediately; labels are resolved
        // after options load.
    ...
    } else if (!this._isMultiSelect && this._projectOptions === undefined) {
      this._loadId = Symbol();
      const select = this.querySelector(".rp-input");
      if (select) select.disabled = true;
      this._attachProgrammeWatcher();
  ...
  _attachProgrammeWatcher() {
    if (this._isMultiSelect) {
      // Multi-select ignores programme filtering.
      this._fetchOptions(null, this._loadId);
      return;
    }
  ...
  async _fetchOptions(programmeCode, id) {
    try {
      ...
      const url =
        programmeCode && !this._isMultiSelect
          ? `${href}?programme=${encodeURIComponent(programmeCode)}`
          : href;
      ...
      if (this._isMultiSelect) {
        this._initialOptions = projects.map((p) => ({
          id: p.code,
        ...
        // Upgrade any placeholder chips with proper labels now that options are available.
