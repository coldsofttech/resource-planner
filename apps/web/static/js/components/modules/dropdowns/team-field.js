 ...
 *
 * Dropdown (or chip-based multi-select) field pre-wired to the teams API.
 * Active teams are fetched from GET /api/v1/teams/options/ on first connect.
 ...
 *
 * Attributes:
 *   allow-all    – single-select only; prepends an "All Teams" option (value="")
 *                  selected by default; used in filter contexts.
 *                  Ignored when multi-select is set.
 *   unassign     – single-select only; prepends an "Unassign from current team"
 *                  option (value="") selected by default. Ignored when multi-select
 *                  is set. Takes precedence over allow-all label when both are set.
 ...
 *
 * Usage (multi-select):
 *   <team-field id="member-teams" multi-select required col="col-md-6"></team-field>
 *   <team-field id="member-teams" multi-select value='["TEAM-1","TEAM-2"]'></team-field>
 *
 * Reading the multi-select value in JS:
 *   JSON.parse(field.value)  // → ["TEAM-1", "TEAM-2"]
 *   field.values             // → [{id, label, value}, …]
 */
class TeamField extends DropdownField {
  static get observedAttributes() {
    return [...super.observedAttributes, "show-label", "multi-select", "allow-all", "unassign"];
  }
  ...
  attributeChangedCallback(name, oldVal, newVal) {
    if (this._isMultiSelect && name === "value" && this._connected && oldVal !== newVal) {
      ...
      (name === "allow-all" || name === "unassign") &&
      !this._isMultiSelect &&
      this._connected &&
      this._teamOptions !== undefined
    ) {
  ...
  connectedCallback() {
    ...
    if (firstConnect) {
      ...
      if (this._isMultiSelect) {
        ...
        // values are visible immediately; labels are resolved after options load.
  ...
  async _fetchOptions(id) {
    try {
      ...
      if (this._isMultiSelect) {
        this._initialOptions = teams.map((t) => ({
          id: t.code,
          label: t.name,
          value: t.code,
          selected: false,
