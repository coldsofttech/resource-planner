 ...
 *
 * Dropdown (or chip-based multi-select) field pre-wired to the products API.
 * Active products are fetched from GET /api/v1/products/options/ on first connect.
 ...
 *
 * Attributes:
 *   allow-all    – single-select only; prepends an "All Products" option (value="")
 *                  selected by default; used in filter contexts.
 *                  Ignored when multi-select is set.
 *   show-label   – when present, renders "Product" as the visible field label.
 ...
 *
 * Usage (single-select):
 *   <product-field id="product" required col="col-md-6"></product-field>
 *   <product-field id="product" value="PROD-1"></product-field>
 *   <product-field id="filter-product" name="product" allow-all></product-field>
 *
 * Usage (multi-select):
 *   <product-field id="products" multi-select required col="col-md-6"></product-field>
 *   <product-field id="products" multi-select value='["PROD-1","PROD-2"]'></product-field>
 *
 * Reading the multi-select value in JS:
 *   JSON.parse(field.value)  // → ["PROD-1", "PROD-2"]
 *   field.values             // → [{id, label, value}, …]
 */
class ProductField extends DropdownField {
  ...
  get _label() {
    if (this.hasAttribute("show-label")) return this.getAttribute("label") || "Product";
    return super._label;
  }
  ...
  connectedCallback() {
    if (!this.hasAttribute("placeholder")) this.setAttribute("placeholder", "Select product…");
    ...
    } else if (!this._isMultiSelect && this._productOptions === undefined) {
      this._loadId = Symbol();
      const select = this.querySelector(".rp-input");
      if (select) select.disabled = true;
      this._fetchOptions(this._loadId);
  ...
  async _fetchOptions(id) {
    try {
      const { href, method } = API_URLS.products.options();
      ...
      if (this._isMultiSelect) {
        this._initialOptions = products.map((p) => ({
          id: p.code,
          label: p.name,
          value: p.code,
          selected: false,
        ...
        this._productOptions = products.map((p) => ({
          id: p.code,
          label: p.name,
          value: p.code,
          selected: false,
  ...
  _buildSingleOptions() {
    ...
    return [
      ...(hasAllOpt
        ? [{ id: "", label: "All Products", value: "", selected: true, disabled: false }]
        : []),
  ...
  _setFetchError() {
    if (this._isMultiSelect) {
      ...
      if (errEl) {
        errEl.textContent = "Could not load products. Refresh the page to retry.";
        errEl.hidden = false;
      }
      ...
      if (select) {
        ...
        select.innerHTML = '<option value="" disabled selected>Could not load products</option>';
      }
      if (errEl) {
        errEl.textContent = "Could not load products. Refresh the page to retry.";
        errEl.hidden = false;
...
customElements.define("product-field", ProductField);
