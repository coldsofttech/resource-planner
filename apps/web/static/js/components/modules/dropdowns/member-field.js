 ...
 *
 * Dropdown (or chip-based multi-select) field pre-wired to the members API.
 * Active members are fetched from GET /api/v1/members/?page_size=200 on first connect.
 ...
 *
 * Options render as the member's display name (or email / code as fallback).
 *
 * Attributes:
 *   allow-all    – single-select only; prepends an "All Members" option (value="")
 ...
 *
 * Usage (multi-select):
 *   <member-field id="sprint-members" multi-select required col="col-12"></member-field>
 *   <member-field id="sprint-members" multi-select value='["MBR-0001","MBR-0002"]'></member-field>
 *
...
class MemberField extends DropdownField {
  ...
  async _fetchOptions(id) {
    try {
      ...
      const mapped = members.map((m) => ({
        id: m.code,
        label: m.display_name || m.email || m.code,
        value: m.code,
        selected: false,
        disabled: false,
      }));
  ...
  _updateDropdown() {
    ...
    dropdown.innerHTML = items.length
      ? items.join("")
      : `<div class="rp-ms-empty">No options found</div>`;
