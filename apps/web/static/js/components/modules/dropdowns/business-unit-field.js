...
/* BusinessUnitField  <business-unit-field>
 *
 * Dropdown (or chip-based multi-select) field pre-wired to the business units API.
 * Active business units are fetched from GET /api/v1/bu/options/ on first connect.
 ...
 *
 * Attributes:
 *   allow-all    – single-select only; prepends an "All Business Units" option
 *                  (value="") selected by default; used in filter contexts.
 *                  Ignored when multi-select is set.
 *   show-label   – when present, renders "Business Unit" as the visible field label.
 ...
 *
 * Usage (single-select):
 *   <business-unit-field id="bu-field" required col="col-md-6"></business-unit-field>
 *   <business-unit-field id="bu-field" value="BU-1"></business-unit-field>
 *   <business-unit-field id="filter-bu" name="bu" allow-all></business-unit-field>
 *
 * Usage (multi-select):
 *   <business-unit-field id="bu-field" multi-select required col="col-md-6"></business-unit-field>
 *   <business-unit-field id="bu-field" multi-select value='["BU-1","BU-2"]'></business-unit-field>
 *
 * Reading the multi-select value in JS:
 *   JSON.parse(field.value)  // → ["BU-1", "BU-2"]
...
class BusinessUnitField extends DropdownField {
  ...
  get _label() {
    if (this.hasAttribute("show-label")) return this.getAttribute("label") || "Business Unit";
    return super._label;
  }
  ...
  connectedCallback() {
    if (!this.hasAttribute("placeholder"))
      this.setAttribute("placeholder", "Select business unit…");
    ...
    if (firstConnect) {
      ...
      if (this._isMultiSelect) {
        ...
        this._selectedValues = pending.map((code) => ({
          id: code,
          label: code,
          value: code,
        }));
  ...
  async _fetchOptions(id) {
    try {
      const { href, method } = API_URLS.businessUnits.options();
      ...
      if (this._isMultiSelect) {
        this._initialOptions = units.map((u) => ({
          id: u.code,
          label: u.name,
          value: u.code,
          selected: false,
  ...
  _setFetchError() {
    if (this._isMultiSelect) {
      ...
      if (errEl) {
        errEl.textContent = "Could not load business units. Refresh the page to retry.";
        errEl.hidden = false;
      }
      ...
      if (select) {
        ...
        select.innerHTML =
          '<option value="" disabled selected>Could not load business units</option>';
      }
      if (errEl) {
        errEl.textContent = "Could not load business units. Refresh the page to retry.";
        errEl.hidden = false;
...
customElements.define("business-unit-field", BusinessUnitField);
